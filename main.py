...
def generate_breathe(color, rate, brightness):
    prefix = '11ff0e3b00'
    suffix = '000000000000'
    mode = '03'
    rate = '{:04x}'.format(max(100, min(65535, int(rate))))
    color = color.lower()
    brightness = '{:02x}'.format(max(1, min(100, int(brightness))))
    data = color + rate + '00' + brightness + '00'
    full_command = prefix + data + suffix
    return full_command


def loop_colors(colors, rate = 5000):
    while True:
        for color in colors:
            set_led_breathe(
                process_color(str(color)),
                process_rate(str(rate)),
                process_brightness('100')
            )
...
if __name__ == '__main__':
    ...
    rate = 5000
    loop_colors(colors, 5000)
        # set_led_breathe('FFFFFF', '10000', '45')